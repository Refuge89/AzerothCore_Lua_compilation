local UnitEntry = 600000
 
function Trainer_Gossip(event, player, unit)
            if (player:IsInCombat() == true) then
                    player:SendAreaTriggerMessage("You can not use in combat!")
            else
...
if (player:GetLevel() >= 0) then
        ...
        player:GossipMenuAddItem(0,"|TInterface\\icons\\Achievement_BG_returnXflags_def_WSG.png:30:30:-15:0|t Reset Talents",0,11)
        player:GossipMenuAddItem(0,"|TInterface\\icons\\Ability_Vehicle_LoadSelfCatapult.png:30:30:-15:0|t Close ",0,999)
...
if (intid < 11 and intid > 0) then
        if (player:ModifyMoney(0) == false) then
                intid = 0
                player:SendBroadcastMessage("You dont have GOLD.!")
